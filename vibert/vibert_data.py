...
def convert_examples_to_features(
    ...
    """
    def get_padded_roberta(input_ids, attention_mask, max_length, pad_token,
    ...
    def label_from_example(example: InputExample):
        if output_mode == "classification":
            return label_map[str(example.label)]
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if(model_type == 'roberta'):
            ...
            input_ids, attention_mask = get_padded_roberta(input_ids,\
...
class AitaProcessor(DataProcessor):
    ...
    def get_validation_examples(self, df):
        """See base class."""
...
def get_metrics(preds, labels):
    ...
    return {"acc": simple_accuracy(preds, labels), "mcc": mcc(preds, labels), "f1Score": f1_score(labels, preds)}
    
def get_confusion_matrix(preds, labels):
    return confusion_matrix(labels, preds).ravel()
